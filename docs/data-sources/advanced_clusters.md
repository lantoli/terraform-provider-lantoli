...
* `disk_iops` - Target IOPS (Input/Output Operations Per Second) desired for storage attached to this hardware. For Gen 2 AWS instance sizes (such as `M30_GEN_2`) with `ebs_volume_type` set to `STANDARD`, this value is configurable between 3,000 and 80,000 IOPS. This parameter defaults to the cluster tier's standard IOPS value.
